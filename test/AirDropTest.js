...
const genLeaves = (addresses, amounts) => {
  let data = [];
  for (let i = 0; i < addresses.length; i++) {
    let leaf = abi.soliditySHA3(['address', 'uint256'],[addresses[i], amounts[i]])
    data.push(leaf)
  }
...
contract('MerkleAirDrop', (accounts) => {
  ...
  const leaves = genLeaves(addresses, amounts);
  ...
  it('deploys with the Merkle root', async () => {
    const mRoot = await merkleAirDrop.mRoot.call()
    assert.equal(mRoot, hexRoot)
  })
